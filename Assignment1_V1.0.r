...
###  kw_min_min -1 as o
sharing_data_f1$kw_min_min[sharing_data_f1$kw_min_min == -1]<-0
summary(sharing_data_f1$kw_min_min)

### Binning of data
## n_token_title 13 bins 
cuts<-bins(sharing_data_f1$n_tokens_title,13,minpts =  1, exact.groups = FALSE)
sharing_data_f1$n_tokens_title_f<-as.integer(cut(sharing_data_f1$n_tokens_title, bins.getvals(cuts)))
## average_token_length 2 bins
cuts<-bins(sharing_data_f1$average_token_length,2,minpts =  1, exact.groups = FALSE)
sharing_data_f1$average_token_length_f<-as.integer(cut(sharing_data_f1$average_token_length, bins.getvals(cuts)))

## num_videos 5 bins
cuts<-bins(sharing_data_f1$num_videos,5,minpts =  1, exact.groups = FALSE)
sharing_data_f1$num_videos_f<-as.integer(cut(sharing_data_f1$num_videos, bins.getvals(cuts)))
## num_keywords 8 bin (generated 7)
cuts<-bins(sharing_data_f1$num_keywords,8,minpts =  1, exact.groups = FALSE)
sharing_data_f1$num_keywords_f<-as.integer(cut(sharing_data_f1$num_keywords, bins.getvals(cuts)))
## kw_min_min 3 bins (generated 2)
cuts<-bins(sharing_data_f1$kw_min_min,3,minpts =  1, exact.groups = FALSE)
sharing_data_f1$kw_min_min_f<-as.integer(cut(sharing_data_f1$kw_min_min, bins.getvals(cuts)))
## kw_max_max 4 bins
cuts<-bins(sharing_data_f1$kw_max_max,4,minpts =  1, exact.groups = FALSE)
sharing_data_f1$kw_max_max_f<-as.integer(cut(sharing_data_f1$kw_max_max, bins.getvals(cuts)))
...
col_name<-colnames(train[,-c(59,66)])
col_name_base<-col_name[1:56]
## xgboost
